...
class UserToken extends Model
{
    ...
    const TOKEN_PERIOD = 15; //token有效期

    private static $tick = 0; // token 时间
    private static $app_id = ''; // 截取token字符串获取的appid,某些接口没有
     ...
     * @return string
     */
    public static function updateToken($uid, $app_id = '')
    {
        return self::setToken($uid, $app_id);
    }
     ...
     * @return string
     */
    private static function setToken($uid, $app_id = '')
    {
        $app_id = !$app_id ? request()->header('ua-app-id', '20082')  : $app_id;
     ...
     * @return string
     */
    public static function _getCacheKey($uid, $app_id = '')
    {
        $cache_key = 'token:' . $uid;
        return  !$app_id ? $cache_key : $cache_key . '_' . $app_id ;
     ...
     * @return mixed
     */
    public static function getToken($uid, $token, $app_id = '')
    {
        // 根据token获取appid
...
//        return self::_recacheToken($uid, $app_id);
     ...
     */
    private static function _dealAppId($token, $app_id = '')
    {
     ...
     * @return null|string
     */
    private static function _recacheToken($uid, $app_id)
    {
     ...
     * @return mixed
     */
    private static function _getTokenFromModel($uid, $app_id)
    {
        $arrWhere = [
            'uid'    => $uid,
            'app_id' => $app_id ?: '20082'
        ];
        return UserToken::query()->where($arrWhere)->select(['token'])->first();
    }
     ...
     * @return bool
     */
    public static function delToken($uid, $app_id = '20082')
    {
        $cache_key = self::_getCacheKey($uid, $app_id);
     ...
     * @return string|null
     */
    public static function getUserToken($uid, $app_id = '')
    {
        $cache_key = self::_getCacheKey($uid, $app_id);
        $app_token = Cache::store('token')->get($cache_key);
        if (!$app_token){
